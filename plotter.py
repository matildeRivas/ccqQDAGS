...
def plot_config():
    ...
    fig.suptitle('Times for Different Decompositions in original qdag structure')
...
def plot_times():
    patterns = ["j3_ghd" "t3_ghd" "ti3_ghd" "j4_ghd" "ti4_ghd" "t4_ghd" "triangle_tadpole" "square_tadpole" "bowtie" "triangle_barbell" "square_barbell" "penta_barbell"]
    medianprops = dict(linestyle='-.', linewidth=2.5, color='black')
    ...
    for i, p in enumerate(patterns):
        if p in multi:
            df1 = pd.read_csv(f"ha_outputs_time/{p}_ghd_yk_1.csv", names=["time"])
            df2 = pd.read_csv(f"ha_outputs_time/{p}_ghd_yk_2.csv", names=["time"])
            df3 = pd.read_csv(f"ha_outputs_time/{p}_ghd_yk_3.csv", names=["time"])
            ...
            df1p = pd.read_csv(f"ha_outputs_time/{p}_ghd_yk_par_1.csv", names=["time"])
            df2p = pd.read_csv(f"ha_outputs_time/{p}_ghd_yk_par_2.csv", names=["time"])
            df3p = pd.read_csv(f"ha_outputs_time/{p}_ghd_yk_par_3.csv", names=["time"])
            ...
            df_mj = pd.read_csv(f"ha_outputs_time/{p}_ghd_mj.csv", names=["multijoin"])
        else:
            yk_res = pd.read_csv(f"ha_outputs_time/{p}_yk.csv", names=["GHD"])
            ykp_res = pd.read_csv(f"ha_outputs_time/{p}_yk_par.csv", names=["GHD par"])
            df_mj = pd.read_csv(f"ha_outputs_time/{p}_mj.csv", names=["multijoin"])
